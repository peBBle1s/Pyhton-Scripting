# gui.py
# [Keep all imports and Theme/Colors sections exactly as they are]
...
from router import (
    ...
    set_app_device, set_app_device_by_pid, toggle_mute, open_windows_audio_settings,
...
class PebXGUI(ctk.CTk):
    # [Keep __init__, start_device_watchdog, and build_ui exactly as they are]
    def __init__(self):
        super().__init__()
        try:
        ...
        def wndproc(hwnd, msg, wparam, lparam):
            if msg == WM_DEVICECHANGE:
                if wparam == DBT_DEVNODES_CHANGED:
                    logger.info("[METABOLISM] Hardware change detected. Triggering auto-sync.")
    ...
    def build_ui(self):
        ...
        self.title_label = ctk.CTkLabel(title_row, text="PEBX  •  SIGNAL MATRIX", text_color=ACCENT, font=("Segoe UI", 22, "bold"))
        self.title_label.pack(side="left", anchor="w")
        self.pulse_indicator = ctk.CTkFrame(title_row, width=14, height=14, fg_color=BG_PANEL, corner_radius=4, border_width=1, border_color=BORDER)
        self.pulse_indicator.pack(side="left", padx=12, pady=4)
        self.status_label = ctk.CTkLabel(header, text="● ENGINE ACTIVE  •  DEFAULT: —", text_color="#4CAF50", font=("Segoe UI", 12))
        self.status_label.pack(anchor="w", pady=(6, 10))
        self.tabview = ctk.CTkTabview(self, fg_color=BG_MAIN, segmented_button_selected_color=ACCENT, segmented_button_selected_hover_color="#00B8CC", text_color="white")
    ...
    # [Keep gui_create_profile, gui_delete_profile, refresh_profiles exactly as they are]
    ...
    def save_to_profile_matrix(self):
        ...
        app_label = self.profile_app_dropdown.get()
        device_name = self.profile_device_dropdown.get()
        
        if app_label in self.apps and device_name in self.devices:
            exe_name, pid = self.apps[app_label]
    ...
    # [Keep _foreground_watcher_loop, _save_auto_profile_with_pulse exactly as they are]
    ...
    def _save_auto_profile_with_pulse(self):
        app_label = self.app_dropdown.get()
    ...
    # [Keep state management, reports, updates exactly as they are]
    def load_state(self):
        ...
        try:
            ...
            for app_label, dev_name in routes.items():
                if app_label in self.apps and dev_name in self.devices:
                    self.saved_app_routes[app_label] = dev_name
                    exe_name, pid = self.apps[app_label]
                    set_app_device_by_pid(self.devices[dev_name], pid)
                    logger.info(f"[MEMORY] Restored Route: {app_label} -> {dev_name}")
        except Exception as e:
            logger.error(f"[MEMORY] Failed to load state: {e}")
    def save_state(self):
        state = {"global_device": self.global_device_dropdown.get(), "app_routes": self.saved_app_routes}
        try:
    ...
    def refresh_apps(self):
        ...
        if self.apps:
            # Step 2: Labels now contain PIDs for surgical selection
            labels = list(self.apps.keys())
            self.app_dropdown.configure(values=labels)
            self.profile_app_dropdown.configure(values=labels)
            if self.app_dropdown.get() not in labels: self.app_dropdown.set(labels[0])
            if self.profile_app_dropdown.get() not in labels: self.profile_app_dropdown.set(labels[0])
        else:
    ...
    # ---------- SURGICAL PER APP ROUTING (STEP 2) ----------
    def apply_app_routing(self):
        ...
        if app_label in self.apps and device_name in self.devices:
            ...
            # Use PID-based routing for exact precision
            set_app_device_by_pid(device_id, pid)
            self.saved_app_routes[app_label] = device_name